...
if($mode=="add")
  ...
  if($act=="add"&&$_SESSION['balls']>=500)
    ...
    if(empty($error))
      ...
      if($_SESSION['balls']>=$balls_us)
        {
        if($_SESSION['balls']>=500 && $_SESSION['balls']<700 && $ban_time>172800) $ban_time=172800;
	$getbans=mysql_query("select mute_settime from chat_banlist where user='".urldecode($ban_l)."'");
	$bans=mysql_num_rows($getbans);
        if($bans>0)
          {
          mysql_query("update chat_banlist set user='$ban_l',ip='$ban_ip',ban='$_SESSION[suser]',type='$type',reson='$reson',mute_time=mute_time+'$ban_time' where user='".urldecode($ban_l)."'") or die(mysql_error());
          }
        else
          mysql_query("insert into chat_banlist values ('','$ban_l','$ban_ip','$_SESSION[suser]','$type','$reson','$ban_time', '$time')") or die(mysql_error());
        mysql_query("update chat_onliners set ban='1',upd='$time' where user='$ban_l' $add");
        if($ban_time<3600)
          {
          $ban_time_t=ceil($ban_time/60);
          $temp_ban=my_bcmod($ban_time_t,10);
          if($temp_ban==1) $ban_time_t.=" минуту";
          elseif(($temp_ban==2||$temp_ban==3||$temp_ban==4)&&my_bcdiv($ban_time_t,10)!=1) $ban_time_t.=" минуты";
        ...
        elseif($ban_time>=3600&&$ban_time<86400)
          ...
          elseif(($temp_ban==2||$temp_ban==3||$temp_ban==4)&&my_bcdiv($ban_time_t,10)!=1) $ban_time_t.=" часа";
        ...
        elseif($ban_time>=86400&&$ban_time<31536000)
          ...
          elseif(($temp_ban==2||$temp_ban==3||$temp_ban==4)&&my_bcdiv($ban_time_t,10)!=1) $ban_time_t.=" дн€";
        ...
        elseif($ban_time>=31536000)
          ...
          $temp_ban=my_bcmod($ban_time_t,10);
          if($temp_ban) $ban_time_t.=" год";
          elseif(($temp_ban==2||$temp_ban==3||$temp_ban==4)&&my_bcdiv($ban_time_t,10)!=1) $ban_time_t.=" года";
          else $ban_time_t.=" лет";
          }
        @$reson_ban=mysql_result(mysql_query("select categ from chat_rules_cats where id='$reson'"),0,'categ');
        @$id_user=mysql_result(mysql_query("select id from chat_users where user='$ban_l'"),0,'id');
        ...
        mysql_query("update chat_banstat set count=count+1 where type='ban' and id_user='$id_user'") or die(mysql_error());
        }
      else
        {
  ...
  elseif($_SESSION['balls']>=500)
    ...
    <?if($_SESSION['balls']<700) echo "<input type=hidden name=\"ban_login\" value=\"$login\">";?>
    <input type="hidden" name="act" value="add">
    <tr valign="top">
    <td align="right"><b><?echo$lang['nick'];?>:</b></td>
    <td><input type="checkbox" name="c_login" checked <?if($_SESSION['balls']<700)echo"disabled";?>>&nbsp;<input type="text" name="ban_login" style="width:400;" value="<?echo$login;?>" <?if($_SESSION['balls']<700)echo"disabled";?>></td>
    </tr>
    <tr valign="top">
    <td align="right"><b><?echo$lang['ip'];?>:</b></td>
    <td><input type="checkbox" name="c_ip" <?if($_SESSION['balls']<700)echo"disabled";?>>&nbsp;<input type="text" name="ban_ip" style="width:400;" value="<?echo$ips;?>" <?if($_SESSION['balls']<700)echo"disabled";?>></td>
    </tr>
    <tr valign="top">
    ...
    <td><select name="ban_time" style="width:422;">
    <option value="300">«абанить на 5 минут
    <option value="900">«абанить на 15 минут
    <option value="1800">«абанить на 30 минут
    <option value="3600">«абанить на 1 час
    <option value="5200">«абанить на 2 часа
    <option value="18000">«абанить на 6 часов
    <option value="43200">«абанить на 12 часов
    <option value="86400">«абанить на 1 день
    <option value="172800">«абанить на 2 дн€
    <?if($_SESSION['balls']>=700){?>
    <option value="604800">«абанить на 1 неделю
    <option value="604800">«абанить на 1 неделю
    <option value="1209600">«абанить на 2 недели
    <option value="2678400">«абанить на 1 мес€ц
    <option value="999999999">«абанить на ~ часов<?}?></select></td>
    </tr>
    <tr valign="top">
    <td></td>
...
elseif($mode=="addwarn"&&$_SESSION['balls']>=500)
  ...
  if($act=="add"&&$_SESSION['balls']>=500)
    ...
    if(empty($error))
      ...
      @$id_user=mysql_result(mysql_query("select id from chat_users where user='$warn_user'"),0,'id');
      if($_SESSION['balls']>$balls_us)
        {
        $content=substr($content,0,60);
        $time=time();
        mysql_query("insert into chat_messages (room, user, message, time) values ('$_SESSION[room]','$bot_name','”частник <b>$warn_user</b> получил предупреждение от модератора <b>$_SESSION[suser]</b>. ќснование: <b>$content</b>','$time');") or die(mysql_error());
        mysql_query("update chat_banstat set count=count+1 where type='warn' and id_user='$id_user'") or die(mysql_error());
        }
      ?><html><head></head><body><script language=javascript>window.close();</script></body></html><?
      }
    }
  ...
  if(!empty($unban)&&$_SESSION['balls']>=500)
    ...
    if(empty($error))
      ...
      if($_SESSION['balls']>=$balls_mod)
        ...
        mysql_query("insert into chat_messages (room, user, message, time) values ('$_SESSION[room]','$bot_name','— участника <b>".urldecode($unban)."</b> был сн€т бан модератором <b>$_SESSION[suser]</b>.','$time')") or die(mysql_error());
        mysql_query("insert into chat_logs values ('','$unban','$_SESSION[suser]','<b>сн€т бан</b>','$time')");
        }
      else
        {
        ...
        $time=time();
        mysql_query("update chat_onliners set ban='1',upd='$time' where user='".urldecode($_SESSION['suser'])."'");
        mysql_query("insert into chat_messages (room, user, message, time) values ('$_SESSION[room]','$bot_name','”частник <b>".urldecode($_SESSION['suser'])."</b> был забанен модератором <b>$bot_name</b> за <b>попытку разбанить бан старшего модератора</b> на <b>$ban_time_t</b>.','$time')") or die(mysql_error());
