...
elseif($mode=="write"&&isset($_SESSION['suser']))
  ...
  if($act=="send")
    ...
    <td class=alt2 width=80%><input type=text name=userto_new size=25 onchange="checks('clogin2',document.form.userto_new.value);"> <span id="clogin2" style="color:#ff0000;font-weight:bold;font-size:11px"></span>&nbsp;
    <a href=# onclick="window.open('mail.php?mode=search','','width=300,height=100,top=0,left=0,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=yes,resizable=no');">Искать пользователя</a></td>
    </tr>
    <tr>
    ...
    <input type=button value="<?echo$lang['sendletter'];?>" onClick="if (check2()) submit();"></td>
    </tr>
    </table></form>
...
elseif($mode=="search"&&isset($_SESSION['suser']))
  {
  include("design/header.tpl");?>
  <script language=JavaScript type=text/javascript src="design/js.php"></script>
  <form method=post name=form>
  <input type=hidden name=act value=search>
  <input type=text name=user_s> <input type=submit value="Искать"><br>
  <?
  if($act=="search")
    {
    $query_search=mysql_query("select user from chat_users where user LIKE '%".$user_s."%'");
    echo "<select name=user_list class=smallfont>";
    while($arr_search=mysql_fetch_array($query_search))
      {
      echo "<option value=\"$arr_search[user]\">$arr_search[user]</option>";
      }
    echo "</select>";
    echo "<input type=button onClick=\"add_user(this.form.user_list.options[this.form.user_list.selectedIndex].value,";
    if(isset($field)&&$field=="newmoder") echo "'newmoder'";
    else echo "'userto_new'";
    echo ");return false;\" value=\"Вставить\" class=smallfont>";
    }
  echo "</form>";
