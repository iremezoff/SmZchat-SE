...
if(isset($_SESSION['suser']))
  ...
  if($resultcnt==1)
    ...
    $now=time();
    if($now-$_SESSION['lmess']>=600 && $_SESSION['status']!=2)
      {
      $xstatus=mysql_query("select text from chat_xstatus where id='2'");
      list($text_x)=mysql_fetch_row($xstatus);
      $_SESSION['status']=2;
      mysql_query("update chat_onliners set status='2',xtext='$text_x',upd='$now' where user='".addslashes($_SESSION['suser'])."'");
      }
	...
	$output.="mess|no|$fon|$msgs[color]|$date|$msgs[user]|$msgs[userto]|$msgs[message]|$msgs[bnick]|$msgs[cnick]:::";
        }
      else
        {
        if((trim($msgs['userto'])==trim($qlogin) || trim($msgs['user'])==trim($qlogin))&&$ign<1)
          {
          if($msgs['color']==$color['bgcolor']) $msgs['color']="#00ff00";
          $output.="mess|yes|$fon|$msgs[color]|$date|$msgs[user]|$msgs[userto]|$msgs[message]|$msgs[bnick]|$msgs[cnick]:::";
          }
        }
      }
    ...
    if($_SESSION['upd']!=$getupd)
      ...
      $_SESSION['upd']=$getupd;
      $output.="clear_u:::";
      $query="select * from chat_onliners where room='$_SESSION[room]' order by ";
      if($_SESSION['sortby']=="nick") $query.="user";
      elseif($_SESSION['sortby']=="sex") $query.="sex";
      else $query.="lastactivity";
      $query_us=mysql_query($query);
      while($array_us=mysql_fetch_array($query_us))
        ...
        if($_SESSION['sid']==-1) $place=-1;
        else
          @$place=mysql_result(mysql_query("select id from chat_level where mess<='$array_us[mess]' order by mess desc limit 1"),0,'id');
        $output.="usrs|$array_us[user]|$array_us[balls]|$array_us[status]|$array_us[xtext]|$ign|$array_us[ban]|$array_us[ip]|$array_us[sid]|$place|$array_us[sex]|#$array_us[colornick]|$array_us[bnick]:::";
        }
